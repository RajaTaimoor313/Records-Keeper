...
class IncomeData {
  ...
  static IncomeData fromMap(Map<String, dynamic> map) {
    return IncomeData(
      date: map['date'],
...
class IncomeTab extends StatefulWidget {
  ...
  const IncomeTab({super.key, required this.isViewMode});
...
class _IncomeTabState extends State<IncomeTab> {
  final List<String> categoryOptions = ['Sales & Recovery', 'Other Income'];
  ...
  List<IncomeData> incomeRecords = [];
  List<IncomeData> filteredRecords = [];
  bool isLoading = true;
  ...
  void _checkDateChange() {
    ...
    if (lastKnownDate != null && lastKnownDate != currentDate) {
      ...
      _loadIncomeRecords().then((_) {
        setState(() {
          // Filter to show only current date's records
          filteredRecords = incomeRecords.where((record) => record.date == currentDate).toList();
        });
      });
    }
  }
  ...
  Future<void> _loadIncomeRecords() async {
    ...
    try {
      final records = await DatabaseHelper.instance.getIncomes();
      final currentDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
      
      setState(() {
        incomeRecords = records
            .map((record) => IncomeData.fromMap(record))
            .toList();
      ...
      print('✖ Error loading income records: $e');
      setState(() {
        isLoading = false;
      });
    }
  ...
  void _filterRecords() {
    setState(() {
      filteredRecords = incomeRecords.where((record) {
        bool categoryMatch =
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      await DatabaseHelper.instance.insertIncome(income.toMap());
