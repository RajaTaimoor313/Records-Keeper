...
class _IncomeHistoryScreenState extends State<IncomeHistoryScreen> {
  ...
  Future<List<Map<String, dynamic>>> _getIncomesByDate(String date) async {
    final records = await DatabaseHelper.instance.getIncomes();
    return records.where((e) => e['date'] == date).toList();
  }

  void _showIncomeDetailDialog(BuildContext context, String date, List<Map<String, dynamic>> records) {
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(flex: 2, child: Text(formatter.format(e['amount']), style: const TextStyle(color: Colors.green, fontWeight: FontWeight.w600, fontSize: 14), textAlign: TextAlign.right)),
                            ],
                          ),
                        ),
                      )),
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () async {
                      final records = await _getIncomesByDate(date);
                      _showIncomeDetailDialog(context, date, records);
                    },
                  ),
                );
              },
